...
const Card = ({ data, index }) => {
  ...
  return (
    <div
      className="group col-span-4 sm:col-span-2 lg:col-span-1 flex flex-col gap-2 p-4 border border-[#00000013] hover:shadow-[0px_3.33px_4.44px_0px_#00000012] rounded-md cursor-pointer"
      onClick={() => {
        router.push(`/articles/${data.category}/123`);
      }}
    >
      <div className="max-h-52 h-48 overflow-hidden rounded-md">
        <img
          src={`/eccomerce/${data.image}`}
          className="h-full w-full object-cover rounded-md group-hover:scale-105 duration-300"
        />
      </div>
